...
SUBSTITUTIONS = [
    ...
    # Additional project-specific substitutions
    ('{{API_DOCS_LOCATION}}', 'docs/api'),
    ('{{API_INTEGRATION_DESCRIPTION}}', 'Integrates with MyMarketingPro REST API via fetch'),
    ('{{API_PATTERN_DESCRIPTION}}', 'Services in src/services/ encapsulate all API calls'),
    ('{{API_PATTERN}}', 'Service class pattern'),
    ('{{API_ROUTE_PATTERN}}', '/api/v1/{resource}'),
    ('{{AUTH_DESCRIPTION}}', 'Authentication via apiKey plugin option passed to services'),
    ('{{COMPONENT_NAMING_CONVENTION}}', 'PascalCase (e.g., MmpButton.vue)'),
    ('{{COMPONENT_PATTERN_DESCRIPTION}}', 'Composition API with provide/inject for plugin-level state'),
    ('{{COMPONENT_PATTERN}}', 'Composition API'),
    ('{{COMPONENT_STRUCTURE_RULE}}', 'Use Composition API with `<script setup lang="ts">`'),
    ('{{COMPONENT_SYNTAX_RULE}}', 'Use `<script setup lang="ts">` for all SFCs'),
    ('{{COVERAGE_THRESHOLD}}', '80%'),
    ('{{DATA_ACCESS_PATTERN}}', 'Service layer in src/services/'),
    ('{{DATA_FLOW_DESCRIPTION}}', 'Components → Composables → Services → API'),
    ('{{DEPLOY_COMMAND}}', 'npm publish'),
    ('{{DEPLOY_PLATFORM}}', 'npm'),
    ('{{FILE_NAMING_CONVENTION}}', 'kebab-case for files, PascalCase for Vue components'),
    ('{{FORMAT_COMMAND}}', 'npm run format'),
    ('{{FUNCTION_NAMING_CONVENTION}}', 'camelCase'),
    ('{{INTEGRATION_TEST_DIR}}', 'tests/integration'),
    ('{{INTEGRATION_TEST_PATTERN}}', '*.integration.test.ts'),
    ('{{ISSUE_PREFIX}}', '#'),
    ('{{ISSUE_TRACKER_URL}}', 'https://github.com/ninjasitm/mymarketingpro-vue/issues'),
    ('{{PRIMARY_PATTERN_EXAMPLE}}', 'useMyMarketingPro() composable'),
    ('{{PRIMARY_PATTERN_NAME}}', 'Composition API Plugin'),
    ('{{SERVICES_DIR}}', 'src/services'),
    ('{{STATE_DIR}}', 'src/composables'),
    ('{{STATE_MANAGEMENT}}', 'Composition API with provide/inject'),
    ('{{STATE_PATTERN_DESCRIPTION}}', 'State managed via composables and Vue\'s provide/inject'),
    ('{{TEST_COVERAGE_COMMAND}}', 'npm run test:coverage'),
    ('{{TEST_FILE_PATTERN}}', '*.test.ts'),
    ('{{TYPE_SAFETY_RULE}}', 'No `any` types except when strictly necessary'),
    ('{{UI_LIBRARY}}', 'N/A (headless plugin)'),
    ('{{UTILITY_LIBRARY}}', 'native ES6+'),
    ('{{UTILITY_PATTERN_DESCRIPTION}}', 'Pure functions in src/utils/'),
    ('{{UTILS_DIR}}', 'src/utils'),
